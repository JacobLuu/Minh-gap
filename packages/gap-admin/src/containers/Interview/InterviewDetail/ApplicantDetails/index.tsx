...
const ApplicantDetails = () => {
  const { interviewDetailData, interViewDetailStatus } = useAppSelector(
    selectInterviewDetailStore,
  );
  const categories = interviewDetailData?.escalated_issues?.map(
  ...
  useEffect(() => {
    if (interViewDetailStatus === REQUEST_STATUS.SUCCESS) {
      if (categories.length > 0) {
        let hasIssue = false;
        const newIssues = [];
        for (let i = 0; i < APPLICANT_DETAILS_TAB_DATA.length; i += 1) {
          if (
            APPLICANT_DETAILS_TAB_DATA[i] &&
            categories.join('').includes(APPLICANT_DETAILS_TAB_DATA[i].value)
          ) {
            newIssues.push({
              id: i,
              label: APPLICANT_DETAILS_TAB_DATA[i].label,
              value: APPLICANT_DETAILS_TAB_DATA[i].value,
              shouldHaveFlag: true,
            });
            if (from && !hasIssue) {
              setSelectedTabValue(APPLICANT_DETAILS_TAB_DATA[i]);
              hasIssue = true;
            }
          } else {
            newIssues.push({
        ...
        setIssues(APPLICANT_DETAILS_TAB_DATA);
      }
    }
  }, [interviewDetailData]);
  ...
  const renderApplicantDetailsContent = () => {
    switch (selectedTabValue?.value) {
      ...
      case APPLICANT_DETAILS_CATEGORY.PERSONAL_PROTECTIVE_EQUIPMENT:
        return <PersonalProtectiveEquipment />;
      case APPLICANT_DETAILS_CATEGORY.ADDRESS_DETAILS:
