const { containerFolder, serviceFolder } = require('../constants');
  ...
  actions(data) {
    ...
    if (data.name) {
      ...
      actions.push({
        type: 'add',
        path: `${
          rootExeDir + packageSourcePath + containerFolder
      ...
      actions.push({
        type: 'append',
        path: `${rootExeDir + packageSourcePath}redux/reducer/index.ts`,
        pattern: REGEX.IMPORT_PLOT,
        template:
          "import {{camelCase name}}Store from '../../containers/{{properCase name}}/reducer';",
        abortOnFail: true,
      });
      ...
      actions.push({
        type: 'append',
        path: `${rootExeDir + packageSourcePath}App.tsx`,
      ...
      actions.push({
        type: 'append',
        path: `${rootExeDir + packageSourcePath}constants/clientPath.ts`,
      ...
      actions.push({
        ...
        pattern: REGEX.IMPORT_PLOT,
        template:
          "import {{properCase name}}Page from './containers/{{properCase name}}/Loadable';",
        unique: true,
        abortOnFail: true,
      });
      // Create the saga
      ...
      actions.push({
        type: 'add',
        path: `${
          rootExeDir + packageSourcePath + serviceFolder
        }{{ properCase name }}Service.ts`,
      ...
      actions.push({
        type: 'append',
        path: `${rootExeDir + packageSourcePath}redux/saga/index.ts`,
        pattern: REGEX.IMPORT_PLOT,
        template:
          "import {{ properCase name }}Saga from '../../containers/{{ properCase name }}/saga';",
        abortOnFail: true,
      });
